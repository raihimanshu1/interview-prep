...
def fix_package_name(package):
    """Fix package name to be a valid Java identifier - COMPLETELY CLEAN"""
    parts = package.split('.')
    fixed_parts = []
    ...
    for part in parts:
        ...
        # Check for numeric prefix at start
        match = re.match(r'^(\d+)-(.+)$', part)
        if match:
            number = match.group(1)
            rest = match.group(2)
            # Remove hyphens from rest and camelCase
            rest = re.sub(r'-([a-z])', lambda m: m.group(1).upper(), rest)
            part = rest + number
        else:
            # Just remove hyphens and camelCase
