...
def fix_package_name(package):
    """Fix package name to be a valid Java identifier - NO DIGITS AT START, NO HYPHENS"""
    parts = package.split('.')
    fixed_parts = []
    ...
    for part in parts:
        ...
        # Handle numeric prefixes: move them to end
        # e.g., "08-rate-limiter" -> "rateLimiter08"
        # e.g., "01-parking-lot" -> "parkingLot01"
        
        # First, handle the numeric prefix if present
        match = re.match(r'^(\d+)-(.+)$', part)
        if match:
            number = match.group(1)
            rest = match.group(2)
            # Convert rest to camelCase
            rest = re.sub(r'-([a-z])', lambda m: m.group(1).upper(), rest)
            # Put number at end
            part = rest + number
        else:
            # No numeric prefix, just remove hyphens
