...
package com.lldtop16.cabBooking;
import java.util.*;
public class CabBookingSystem {
    ...
    */
    static class Cab {
        String number;
        String model;
        Cab(String number, String model) {
            this.number = number;
            this.model = model;
        }
    }
    ...
    */
    static class Driver {
        ...
        Location location;
        boolean available;
        Driver(int id, String name, Cab cab, Location location) {
            this.id = id;
            this.name = name;
            this.cab = cab;
    ...
    */
    static class Ride {
        ...
        Location drop;
        double fare;
        Ride(Rider rider, Driver driver, Location pickup, Location drop) {
            this.rider = rider;
            this.driver = driver;
            this.pickup = pickup;
            this.drop = drop;
    ...
    */
    ...
    ========================================================
    */
    static class NearestDriverStrategy implements DriverMatchingStrategy {
        public Driver findDriver(List<Driver> drivers, Location pickup) {
            Driver selected = null;
            double minDistance = Double.MAX_VALUE;
            for (Driver driver : drivers) {
                if (driver.available) {
                    double distance = driver.location.distance(pickup);
    ...
    */
    interface FareStrategy {
        double calculate(Location start, Location end);
    }
    static class NormalFareStrategy implements FareStrategy {
    ...
    */
    static class CabService {
        ...
        CabService(List<Driver> drivers) {
            this.drivers = drivers;
            strategy = new NearestDriverStrategy();
            fareStrategy = new NormalFareStrategy();
        }
        public Ride bookRide(Rider rider, Location pickup, Location drop) {
            Driver driver = strategy.findDriver(drivers, pickup);
            if (driver == null) {
        ...
        public void completeRide(Ride ride) {
            ride.fare = fareStrategy.calculate(ride.pickup, ride.drop);
            ride.driver.endRide();
        }
    }
    /*
    ...
    */
    public static void main(String[] args) {
        Driver d1 = new Driver(1, "Driver A", new Cab("KA01", "Sedan"), new Location(10, 10));
        Driver d2 = new Driver(2, "Driver B", new Cab("KA02", "SUV"), new Location(20, 20));
        CabService service = new CabService(List.of(d1, d2));
        Rider rider = new Rider(1, "Himanshu");
        Ride ride = service.bookRide(rider, new Location(11, 11), new Location(15, 15));
