...
package com.lldtop16.libraryManagement;
...
public class LibraryManagementSystem {
    ...
    */
    static class BorrowRecord {
        ...
        BorrowRecord(Member member, BookCopy copy) {
            this.member = member;
            this.copy = copy;
        ...
        public long getLateDays() {
            ...
            long days = Duration.between(issueDate, returnDate).toDays();
            /*
    ...
    */
    interface FineCalculator {
        double calculate(long lateDays);
    }
    static class SimpleFineCalculator implements FineCalculator {
        public double calculate(long lateDays) {
            return lateDays * 10;
        }
    }
    /*
    ...
    */
    static class Library {
        ...
        Library(List<BookCopy> copies) {
            this.copies = copies;
            this.records = new ArrayList<>();
        }
        // Search available book
        public BookCopy searchBook(String title) {
            for (BookCopy copy : copies) {
                if (copy.isAvailable() && copy.getBook().getTitle().equalsIgnoreCase(title)) {
                    return copy;
                }
            }
            return null;
        ...
        // Issue book
        public BorrowRecord issueBook(Member member, String title) {
            BookCopy copy = searchBook(title);
            if (copy == null) {
        ...
        // Return book
        public double returnBook(BorrowRecord record, FineCalculator calculator) {
            record.returned();
            record.copy.returnBook();
            long lateDays = record.getLateDays();
    ...
    */
    public static void main(String[] args) {
        ...
        BookCopy copy = new BookCopy("COPY-1", book);
        // Create library
        Library library = new Library(List.of(copy));
        // Member
        Member member = new Member("Himanshu");
        // Issue
        BorrowRecord record = library.issueBook(member, "Clean Code");
        System.out.println("Book issued");
        // Return
        FineCalculator calculator = new SimpleFineCalculator();
        double fine = library.returnBook(record, calculator);
