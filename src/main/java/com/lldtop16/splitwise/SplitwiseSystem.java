    ...
    */
    static class BalanceSheet {
        ...
        public void showBalances() {
            for (User giver : balances.keySet()) {
                for (Map.Entry<User, Double> entry : balances.get(giver).entrySet()) {
                    System.out.println(entry.getKey().getName() + " owes " + giver.getName() + " : " + entry.getValue());
                }
            }
        }
    }
    ...
    */
    static class SplitwiseService {
        ...
        public void addExpense(String description, double amount, User paidBy, List<User> users, SplitStrategy strategy) {
            List<Split> splits = strategy.split(amount, users);
