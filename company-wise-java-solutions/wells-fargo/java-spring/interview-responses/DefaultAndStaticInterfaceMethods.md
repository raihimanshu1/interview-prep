# Default And Static Interface Methods - Interview Response

## What Is It?

Default methods are inherited instance behavior in interfaces. Static interface methods are helpers called on the interface name.
...
In a banking backend, Default And Static Interface Methods matters because correctness, security, concurrency, and observability must survive real production traffic.

If we get it wrong:

```text
Conflicting default methods from two interfaces must be resolved in the class.
...
In an interview, I would say: Default methods are inherited instance behavior in interfaces. Static interface methods are helpers called on the interface name. For example, in a banking backend, Default And Static Interface Methods matters because correctness, security, concurrency, and observability must survive real production traffic. The main production risk is Conflicting default methods from two interfaces must be resolved in the class.
